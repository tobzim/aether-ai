( function( blocks, i18n, element, blockEditor, components, apiFetch ) {
    ...
    blocks.registerBlockType( 'wp-ai/connect-block', {
        title: i18n.__( 'wp AI Optimize', 'wp-ai' ),
        icon: 'edit',
        category: 'widgets',
        attributes: {
        ...
        edit: function( props ) {
            ...
            useEffect( function() {
                apiFetch( { path: '/wp-ai/v1/prompts' } )
                .then( setPrompts )
                .catch( console.error );
            }, [] );
            ...
            var options = [ { label: i18n.__( 'Default', 'wp-ai' ), value: '' } ]
            ...
            return el(
                'div',
                { className: 'wp-ai-block' },
                el( SelectControl, {
                    label:       i18n.__( 'Prompt auswählen', 'wp-ai' ),
                    value:       promptId,
                    options:     options,
                    onChange:    function( v ) { setAttrs( { promptId: v } ); },
                    help:        i18n.__( 'Wähle einen vordefinierten Prompt.', 'wp-ai' )
                } ),
                el( RichText, {
                    tagName:    'p',
                    value:      content,
                    onChange:   function( v ) { setAttrs( { content: v } ); },
                    placeholder: i18n.__( 'Text eingeben…', 'wp-ai' )
                } ),
                isLoading
                    ? el( Spinner )
                    : el(
