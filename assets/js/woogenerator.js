jQuery( document ).ready( function( $ ) {
    const root      = wpAiWoogenerator.root;
    const nonce     = wpAiWoogenerator.nonce;
    ...
    function loadProducts() {
        ...
        fetch( root + 'wp-ai/v1/products', { headers: { 'X-WP-Nonce': nonce } } )
        .then( res => res.json() )
        .then( products => {
            Swal.close();
    ...
    function initGenerate() {
        $( '#woogenerate-button' ).off('click').on( 'click', function() {
            ...
            fetch( root + 'wp-ai/v1/products/generate', {
                method: 'POST',
